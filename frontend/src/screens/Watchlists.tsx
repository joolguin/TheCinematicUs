...
export function Watchlists({ user, onDone, onSwitch }: { user: UserName; onDone: () => void; onSwitch: () => void }) {
  ...
  const updateBtn = (
    <button
      onClick={refresh}
      className="bg-[#111118] text-[#a78bfa] border-[1.5px] border-[#26263a] rounded-[14px] py-[14px] text-[17px] font-medium flex items-center justify-center gap-2"
    >
  ...
  const startBtn = (
    <button
      onClick={onDone}
      disabled={running}
      className="bg-[#7c3aed] text-white rounded-[14px] py-[15px] text-[18px] font-semibold disabled:opacity-40 flex items-center justify-center gap-1.5"
    >
  ...
  return (
    <div className="min-h-screen max-w-[430px] mx-auto flex flex-col px-[18px] pt-3 pb-5 overflow-y-auto animate-fadeUp">
      <div className="mb-[22px]">
        <button onClick={onDone} className="text-[#4a4a62] text-[16px] py-1 flex items-center gap-1"><ArrowLeft size={18} />Volver</button>
      </div>
      ...
      <div className="flex items-center gap-2.5 mb-1.5">
        ...
        <div className="flex-1">
          <p className="text-[14px] text-[#3a3a50] mb-0.5">Hola, {user}</p>
      ...
      {!running && !settled && (
        <div className="flex flex-col gap-2.5">
          <div className="bg-[#111118] border-[1.5px] border-[#26263a] rounded-[16px] px-[18px] py-4">
            {lastUpdated && (
              <p className="text-[#4a4a62] text-[16px] mb-3">Última actualización: {lastUpdated}</p>
            )}
            {entries.length > 0 ? (
              entries.map(([name, r]) => (
                <div key={name} className="flex justify-between mb-1.5 last:mb-0">
                  <span className="text-[#f8f8fa] text-[17px] font-medium">
                    <span
                  ...
                  <span className="text-[#6b6b82] text-[16px]">{r.count} películas</span>
                </div>
              ))
            ) : (
              <p className="text-[#4a4a62] text-[16px]">
          ...
          <div className="bg-[#111118] border-[1.5px] border-[#26263a] rounded-[16px] p-[18px] flex flex-col gap-3">
            ...
            {(entries.length > 0 ? entries.map(([n]) => n) : ['Jo', 'Vale']).map((name) => (
              <div key={name} className="flex justify-between">
                ...
                <span className="text-[#4a4a62] text-[16px]">buscando…</span>
              </div>
            ))}
          </div>
          {startBtn}
      ...
      {settled && (
        <div className="flex flex-col gap-2.5 animate-fadeIn">
          <div className="bg-[#0a1f10] border-[1.5px] border-[#166534] rounded-[16px] px-[18px] py-4">
            <p className="text-[#4ade80] text-[16px] font-medium mb-1 flex items-center gap-1.5"><Check size={18} />Watchlists actualizadas</p>
            {lastUpdated && <p className="text-[#2d5a3d] text-[15px] mb-3">Última actualización: {lastUpdated}</p>}
            {entries.map(([name, r]) => (
              <div key={name} className="flex justify-between mb-1.5 last:mb-2.5">
                <span className="text-[#f8f8fa] text-[17px]">
                  <span
                    className="inline-block w-[7px] h-[7px] rounded-full mr-[7px] align-middle"
                    style={{ background: dotColor(name) }}
                  />
                ...
                <span className={r.ok ? 'text-[#4ade80] text-[16px]' : 'text-[#fbbf24] text-[16px]'}>
                  {r.ok
                    ? `${r.count} películas ✓`
            ...
            {status.status === 'error' && status.result?.error && (
              <p className="text-[#fbbf24] text-[15px]">error — {status.result.error}</p>
            )}
            {reenriched && (
              <p className="text-[#2d5a3d] text-[15px]">Re-enriquecidas: {reenriched.enriched} películas</p>
            )}
          </div>
          {updateBtn}
          {startBtn}
